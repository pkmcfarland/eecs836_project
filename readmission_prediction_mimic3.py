...
if __name__ == "__main__":
    ...
    try:
        logging.basicConfig(
        filename= Path(config["LOG_DIR"]) / f"readmission_prediction_{now}.log",
        filemode="w",
        level=logging.INFO,
    ...
    try:
        ...
        tables = config["READMISSION_PREDICTION"]["TABLES"]
        cache_dir = config["CACHE_DIR"]
        check_points = config["READMISSION_PREDICTION"]["CHECK_POINTS"]
        dev_mode = config["READMISSION_PREDICTION"]["DEV_MODE"]
        splits = [config["READMISSION_PREDICTION"]["SPLITS"]["TRAIN"],
                  config["READMISSION_PREDICTION"]["SPLITS"]["VAL"],
                  config["READMISSION_PREDICTION"]["SPLITS"]["TEST"]]
        models = config["READMISSION_PREDICTION"]["MODELS"]
    except KeyError as e:
    ...
    logger.info("Setting ReadmissionPredictionMIMIC3 task (window=30 days)")
    ...
    for model_name in models:
        ...
        # STEP 5: evaluate on test set — required metrics: AUROC and AUPRC
